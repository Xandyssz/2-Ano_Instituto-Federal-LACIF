...
if (!empty($id)) {
    $result_usuario = "DELETE FROM lacifs93_ifsp_lacif.noticias WHERE idNoticia = $id";
    mysqli_query($conn, $result_usuario);

    if (mysqli_affected_rows($conn)) {
        echo "<script>$(document).ready(function() { $('#msgDelete').modal(); })</script>";
        echo '<meta HTTP-EQUIV="Refresh" CONTENT="2; URL=PainelAdminListarNoticia.php">';
    } else {
